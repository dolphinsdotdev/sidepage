...
- [x] CLI: `serve --no-suffix` — opts out of the dedupe id for BYO-domain apps, routing a bare `<app-name>.<domain>`. `--domain`-only (rejected loud otherwise: local serving has no hostname, `--anon`'s is Cloudflare's), stored by `app register`/`--autoregister` since it decides the replayed URL, and never consumes a `name_bindings.json` id — so dropping the flag later yields the id the app would have had all along
- [x] Core: real name-collision detection for BYO-domain hostnames, with the zone's own DNS as the authority in the absence of a directory service (`sidepage.core.tunnel_manager.assert_hostname_available` as a pre-flight before `serve`/`proxy` launch anything, `_assert_hostname_claimable` again inside `open_byo_tunnel` before the shared `cloudflared` starts, and once more inside `_upsert_cname_record` so no write path can bypass it). Any record on the name that isn't a CNAME to *this* domain's tunnel raises `NameCollisionError` — previously `_upsert_cname_record` would have silently repointed it. Applies to suffixed and `--no-suffix` hostnames alike. Known limits, both per-machine: a second machine sharing the same domain config writes indistinguishable records, and `_domain_lock` can't serialize a cross-machine race
...
- [x] `--detach` / `--json` on `serve` and `proxy` — backgrounding and machine-readable output moved into the CLI itself, replacing the skill's shell wrapper pair. `--detach` re-execs via `python -m sidepage` and treats `registry.register()` as the readiness signal, so "running" means serving rather than "a URL appeared in a log"; a failed launch reports the child's own error and exits 1. Verified end to end against real fixtures and a real tunnel (`tests/test_detach_json.py`)
- [x] Packaged Claude Skill for agents/harnesses (`skills/sidepage-serve/`) — now pure prose over `--detach --json`, with no bundled executables; soft drift check that its documented flags still resolve on the real CLI (`tests/test_skill_docs.py`)
